...
public class GameAIScript : MonoBehaviour {
	...
	private GameObject playerPlacedCube;
	...
	public void removePlayerPawn(string st)
	{
		if(st == "enemy"){
			...
			GameObject removPlayer = null;
			foreach(GameObject obj in players){
				if(!daDDyCubes.Contains(obj.transform.parent.name)){
					if(goingDaddyEnemies.Contains(obj)){
						removPlayer = obj;
						goto ghi;
					}
					else
		...
		ghi:
			...
			if(removPlayer){
				emptyPositions.Add(removPlayer.transform.parent.gameObject);
				GameObject removea = Instantiate(removePS, new Vector3(removPlayer.transform.position.x, removPlayer.transform.position.y, -2.0f), Quaternion.Euler(0, 0, 0)) as GameObject;
				
				DestroyObject(removPlayer);
				...
				if(tempList.Count != 0)
					...
					if(tempList.Count != 0)
						...
						if(tempList.Count != 0)
							...
							tempList.Clear();
							tempList = new ArrayList(getCrossPositions(playerPlacedCube));
							if(tempList.Count != 0)
								index=0;
							else{
								tempList.Clear();
	...
	public void moveComputer(){
		...
		if(tempList.Count != 0){
			foreach(GameObject cub in tempList){
				if(hasPlayerinAdj(cub))
					playerAboutToDaadi = true;
			}
		}
...
//		GameObject cubeDest = GameObject.Find(movablePos[movPawn] as string);
		if((movPawn == null || cubeDest == null) && playerAboutToDaadi){
			foreach(GameObject key in keys){
				...
				foreach(string st in destCubes){
					...
					if(cubeDest && !table.ContainsKey(movPawn.transform.parent.gameObject.name))
						goto asdf;
				}
			}
			playerAboutToDaadi = false;
		}
	...
	//hard level
	public ArrayList getCrossPositions(GameObject playersCube){
		ArrayList positions = new ArrayList();
		Tile scr = playersCube.GetComponent<Tile>();
		foreach(GameObject cross in scr.crossCube){
			if(emptyPositions.Contains(cross)){
				positions.Add(cross);
			}
		}
		return positions;
	...
	public bool hasPlayerinAdj(GameObject obj){
		Tile scrp = obj.GetComponent<Tile>();
		foreach(GameObject cub in scrp.adjusents){
			if(checkIfPlayerExist(cub.name, playerPawn))
				return true;
		}
		return false;
	}
